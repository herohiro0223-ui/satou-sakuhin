    ...
    <div className="min-h-screen bg-cream">
      ...
      {showLightbox && (
        ...
        >
          <button
            onClick={() => setShowLightbox(false)}
            className="absolute top-4 right-4 z-10 text-white/80 bg-black/40 rounded-full p-2"
          >
      ...
      <main className="px-4 pb-8">
        {/* Large image */}
        <div className="mb-4 cursor-pointer" onClick={() => setShowLightbox(true)}>
          <img
            src={artwork.imageUrl}
            alt={artwork.title}
        ...
        <button
            ...
            if (navigator.share) {
              try {
                await navigator.share({
                  title: `${child.name}の さくひん「${artwork.title}」`,
                  text: artwork.memo || `${child.name}の さくひん「${artwork.title}」`,
          ...
          className="w-full py-3 mb-3 bg-sky text-white rounded-full font-medium text-center hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
        >
